...
class TestMain:
    ...
    def test_main_with_clean_repo(self, mock_patch_colors, mock_create_pdfs, mock_repo):
        ...
        test_args = ["genpdf", "test_dir", "--pagesize", "a4", "--showchords", "true"]
    ...
    def test_main_with_dirty_repo(self, mock_print, mock_patch_colors, mock_create_pdfs, mock_repo):
        ...
        with patch.object(sys, "argv", test_args):
            ...
            error_message_found = any("Cannot operate on a repo" in call for call in print_calls)
            assert error_message_found

    @patch("genpdf_butler.__main__.Repo")
    @patch("genpdf_butler.__main__.GenPDF.createPDFs")
    @patch("genpdf_butler.__main__.PatchTextColor.PatchColors")
    def test_main_default_arguments(self, mock_patch_colors, mock_create_pdfs, mock_repo):
        ...
        with patch.object(sys, "argv", test_args), \
            ...
            mock_create_pdfs.assert_called_once_with("/current/dir", "a6", "false")

    @patch("genpdf_butler.__main__.Repo")
    @patch("genpdf_butler.__main__.GenPDF.createPDFs")
    @patch("genpdf_butler.__main__.PatchTextColor.PatchColors")
    def test_main_custom_arguments(self, mock_patch_colors, mock_create_pdfs, mock_repo):
        ...
        test_args = ["genpdf", "/music/folder", "--pagesize", "letter", "--showchords", "true"]
        
        with patch.object(sys, "argv", test_args):
            main()
            
            mock_create_pdfs.assert_called_once_with("/music/folder", "letter", "true")
    ...
    def test_git_repo_initialization(self, mock_repo):
        ...
        with patch.object(sys, "argv", test_args), \
            ...
            mock_repo.assert_called_once_with(path=".", search_parent_directories=True)
    ...
    def test_dirty_files_filtering(self, mock_print, mock_repo):
        ...
        mock_repo_instance.untracked_files = ["new_song.chopro", "other.md", "test.cho"]
        mock_repo.return_value = mock_repo_instance
        ...
        with patch.object(sys, "argv", test_args):
            ...
            # Check that only chopro/cho files are mentioned in error messages
            modified_files_mentioned = any("song.chopro" in call and "config.cho" in call for call in print_calls)
            untracked_files_mentioned = any("new_song.chopro" in call and "test.cho" in call for call in print_calls)
    ...
    def test_argument_parser_configuration(self):
        ...
        # We can't easily test the parser directly, but we can test through sys.argv
        test_cases = [
        ...
        for test_args, expected_args in test_cases:
            with patch.object(sys, "argv", test_args), \
                 ...
                 patch("genpdf_butler.__main__.PatchTextColor.PatchColors"), \
                 patch("genpdf_butler.__main__.GenPDF.createPDFs") as mock_create_pdfs:
