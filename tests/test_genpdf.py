...
class TestCreatePDFs:
    ...
    def test_chordpro_settings_configuration(self):
        ...
        with patch("genpdf_butler.GenPDF.os.path.exists", return_value=False), patch(
            "genpdf_butler.GenPDF.print"
        ) as mock_print:
            createPDFs("nonexistent", "a4", "true")
            mock_print.assert_called_once_with("no such file or folder 'nonexistent'")

    def test_ext_function_returns_lowercase_extension(self):
        """Test that the internal ext function returns lowercase extensions."""
        with patch("genpdf_butler.GenPDF.os.path.exists", return_value=True), patch(
            "genpdf_butler.GenPDF.os.path.isdir", return_value=False
        ), patch("genpdf_butler.GenPDF.subprocess.run") as mock_run:
            # Create a temporary file with uppercase extension
            with tempfile.NamedTemporaryFile(suffix=".CHOPRO", delete=False) as tmp:
                tmp_path = tmp.name
            
            try:
                createPDFs(tmp_path, "a4", "true")
                # Should have been called since the extension matches (case insensitive)
    ...
    @patch("genpdf_butler.GenPDF.os.path.isdir")
    @patch("genpdf_butler.GenPDF.os.path.exists")
    def test_single_cho_file_processing(self, mock_exists, mock_isdir, mock_run):
    ...
    def test_nonexistent_target(self, mock_exists, mock_print):
        ...
        mock_print.assert_called_once_with("no such file or folder 'nonexistent.chopro'")

    def test_parameter_variations(self):
        """Test that different parameters are properly incorporated."""
        with patch("genpdf_butler.GenPDF.os.path.exists", return_value=True), \
